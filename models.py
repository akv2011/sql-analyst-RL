...
class SqlAnalystAction(Action):
    ...
    action_type: str = Field(
        ...,
        description="Type of action: 'execute_sql' to run a query, or 'submit_answer' to submit final answer",
    )
    content: str = Field(
        ...,
        description="SQL query string (for execute_sql) or final answer text (for submit_answer)",
    )
...
class SqlAnalystObservation(Observation):
    ...
    task_id: int = Field(default=1, description="Current task ID (1, 2, or 3)")
